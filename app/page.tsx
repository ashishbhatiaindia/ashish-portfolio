...
import { 
  ...
  Linkedin as LucideLinkedin // Using a specific alias to prevent module conflicts
} from 'lucide-react';

export default function FullPortfolio() {
  const skills = {
    languages: ["Java", "FTL"], [cite: 8]
    frameworks: ["Spring Boot", "Microservices", "REST APIs", "SOAP", "TCP", "Kafka", "RabbitMQ", "GraphQL"], [cite: 9, 30]
    cloud: ["AWS", "Docker", "Kubernetes", "CI/CD", "Jenkins", "Git"], [cite: 9]
    databases: ["PostgreSQL", "MySQL", "MongoDB", "Redis", "Elasticsearch"] [cite: 10]
  };
  ...
  return (
    <main className="min-h-screen bg-[#020617] text-slate-100 selection:bg-blue-500/30 font-sans leading-relaxed">
      ...
      <section className="relative pt-24 pb-16 px-6 max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <h1 className="text-6xl md:text-8xl font-black tracking-tighter mb-4 text-white">
            Ashish Bhatia [cite: 1]
          </h1>
          <p className="text-2xl md:text-3xl text-blue-400 font-bold mb-8 italic">
            Senior Software Engineer & Systems Architect [cite: 4, 5]
          </p>
          
          <div className="flex flex-wrap gap-y-4 gap-x-8 text-slate-400 font-medium border-b border-slate-800 pb-12">
            <a href="mailto:ashishbhatia.india@gmail.com" className="flex items-center gap-2 hover:text-white transition-colors"><Mail size={18}/> ashishbhatia.india@gmail.com [cite: 2]</a>
            <span className="flex items-center gap-2"><Phone size={18}/> 8800847114 [cite: 2]</span>
            <span className="flex items-center gap-2"><MapPin size={18}/> Gurugram, India [cite: 2]</span>
            <a href="https://linkedin.com/in/ashishbhatia" target="_blank" className="flex items-center gap-2 hover:text-white transition-colors"><LucideLinkedin size={18}/> LinkedIn [cite: 2]</a>
          </div>

          <p className="mt-12 max-w-4xl text-xl text-slate-300">
            Senior software engineer with 9+ years of experience designing and building scalable distributed systems[cite: 4]. Expert in microservices, system design, and delivering high-impact solutions for enterprise-scale applications[cite: 5].
          </p>
        </motion.div>
      </section>
      ...
      <section className="relative px-6 max-w-6xl mx-auto mb-32">
        <h2 className="text-xs font-black uppercase tracking-[0.3em] text-blue-500 mb-8">Technical Arsenal [cite: 7]</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {Object.entries(skills).map(([category, items], i) => (
            <div key={category} className="p-6 rounded-2xl bg-slate-900/40 border border-slate-800/50 backdrop-blur-sm">
              <h3 className="text-xs font-black uppercase tracking-widest text-blue-400 mb-4">{category} [cite: 7]</h3>
              <div className="flex flex-wrap gap-2">
                {items.map(item => (
                  <span key={item} className="px-2 py-1 bg-slate-800 rounded text-xs font-bold text-slate-300">{item} [cite: 8, 9, 10]</span>
                ))}
              </div>
            </div>
          ))}
      ...
      <section className="relative px-6 max-w-6xl mx-auto mb-32">
        <h2 className="text-4xl font-black mb-16 flex items-center gap-4 text-white">
          <Terminal className="text-blue-500" /> Professional Experience [cite: 13]
        </h2>
        
        <div className="space-y-20">
          <ExperienceBlock 
            title="Senior Member of Technical Staff" [cite: 14]
            company="Grey Orange" [cite: 15]
            date="April 2025 – Present" [cite: 16]
            bullets={[
              "Lead architecture for microservices processing 100K+ daily transactions[cite: 17].",
              ...
              "Mentoring senior engineers on system design and architectural principles[cite: 20]."
            ]}
          />

          <ExperienceBlock 
            title="Assistant Technical Manager" [cite: 21]
            company="Grey Orange" [cite: 22]
            date="April 2024 – March 2025" [cite: 23]
            bullets={[
              ...
              "Collaborated with product teams to translate requirements into technical solutions[cite: 27]."
            ]}
          />

          <ExperienceBlock 
            title="Senior Software Developer" [cite: 28]
            company="Grey Orange" [cite: 29]
            date="April 2022 – April 2024" [cite: 29]
            bullets={[
              "Built RESTful and GraphQL APIs serving 50+ enterprise clients globally[cite: 30].",
              "Optimized database queries, reducing load by 35%[cite: 31].",
              "Designed fault-tolerant integration layers ensuring 99.9% uptime[cite: 32].",
              "Led migration of monolithic applications to a microservices architecture[cite: 33]."
            ]}
          />

          <ExperienceBlock 
            title="Software Developer" [cite: 34]
            company="Grey Orange" [cite: 35]
            date="July 2019 – March 2022" [cite: 36]
            bullets={[
              "Developed core robotics features using Java and Spring Boot[cite: 37].",
              "Implemented RabbitMQ messaging for improved system resilience[cite: 37].",
              "Automated deployment pipelines and established monitoring infrastructure[cite: 38]."
            ]}
          />

          <ExperienceBlock 
            title="Consultant" [cite: 39]
            company="Zillious Solutions" [cite: 40]
            date="June 2016 – June 2019" [cite: 41, 42]
            bullets={[
              ...
              "Provided technical consultation directly with clients[cite: 45]."
            ]}
          />
        </div>
      </section>
      ...
      <section className="relative px-6 max-w-6xl mx-auto pb-32 grid grid-cols-1 md:grid-cols-2 gap-16 border-t border-slate-900 pt-20">
        <div>
          <h2 className="text-3xl font-black mb-10 flex items-center gap-3 text-white">
            <GraduationCap className="text-blue-500" /> Education [cite: 46]
          </h2>
          <div className="space-y-8">
            <EducationItem 
              degree="Master of Computer Applications (MCA)" [cite: 48]
              uni="University of Delhi" [cite: 47]
              loc="Delhi, India [cite: 53]"
            />
            <EducationItem 
              degree="Bachelor of Computer Applications (BCA)" [cite: 50]
              uni="Makhanlal Chaturvedi National University" [cite: 49]
              loc="Bhopal, India [cite: 54]"
            />
          </div>
        </div>
        ...
        <div>
          <h2 className="text-3xl font-black mb-10 flex items-center gap-3 text-white">
            <Award className="text-blue-500" /> Certifications [cite: 51]
          </h2>
